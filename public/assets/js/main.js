...
function initApp() {
    ...
    Promise.all([
        ...
        // Attendre que les composants dynamiques soient chargés
        setTimeout(() => {
            // Activer les fonctionnalités après le chargement des données et composants
            initializeEvents();
            initializeFilters();
            animateOnScroll();
        }, 300); // Attendre un peu plus longtemps pour s'assurer que tout est chargé
    });
...
async function loadProducts() {
    try {
        // Essayer d'abord les produits améliorés, puis fallback sur les produits de base
        let response;
        try {
            response = await fetch('./data/enhanced-products.json');
        } catch {
...
function attachProductDetailsEvents() {
    document.querySelectorAll('.product-details').forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Utiliser le nouveau modal si disponible, sinon fallback
...
// Charger les composants améliorés
function loadEnhancedComponents() {
    // Charger les scripts des nouveaux composants
